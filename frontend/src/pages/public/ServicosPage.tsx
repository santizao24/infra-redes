...
const services = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1589939705382-55e49310667?w=600',
  },
  {
    icon: Wrench,
    ...
    image: 'https://images.unsplash.com/photo-1541976590-713941681597?w=600',
  },
  {
    icon: Construction,
    title: 'Pavimentação',
          ...
          {services.map((s, i) => (
            <div key={s.title} className={`grid lg:grid-cols-2 gap-8 items-center ${i % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
              ...
              <img
                src={s.image}
                alt={s.title}
                className={`rounded-2xl shadow-lg w-full h-[280px] object-cover ${i % 2 === 1 ? 'lg:order-1' : ''}`}
              />
            </div>
          ))}
        </div>
